...
void main() {
  ...
  testWidgets(
    'should navigate to entertainment screen, fill the form and create a deep link',
    (tester) async {
      // setup app
      await setupApp();
      ...
      final entertainmentTile = find.text('Entertainment');
      await tester.tap(entertainmentTile);
      ...
      // check if start date selection is working
      expect(find.text('Start date (optional)'), findsOneWidget);
      await tester.tap(find.byKey(const Key('StartDateWidget')));
      await tester.pumpAndSettle();
      final startDateToSelect = DateTime.now();
      await tester.tap(find.text(startDateToSelect.day.toString()));
      await tester.tap(find.text('OK'));
      await tester.pumpAndSettle();
      expect(
        find.text(DateFormat('dd/MM/yyyy').format(startDateToSelect)),
        findsOneWidget,
      );

      // check if end date selection is working
      expect(find.text('End date (optional)'), findsOneWidget);
      await tester.tap(find.byKey(const Key('EndDateWidget')));
      await tester.pumpAndSettle();
      final endDateToSelect = startDateToSelect.add(const Duration(days: 10));
      if (endDateToSelect.month != startDateToSelect.month) {
        await tester.tap(find.byIcon(Icons.chevron_right));
        await tester.pumpAndSettle();
      }
      await tester.tap(find.text(endDateToSelect.day.toString()));
      await tester.tap(find.text('OK'));
      await tester.pumpAndSettle();
      expect(
        find.text(DateFormat('dd/MM/yyyy').format(endDateToSelect)),
        findsOneWidget,
      );
      ...
      final categoriesOptionsWidget =
          find.byKey(const Key('CategoriesOptionsWidget'));
