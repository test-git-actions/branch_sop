...
void main() {
  ...
  testGoldens(
    'RestaurantsWidget - Loading state',
    (tester) async {
  ...
  testGoldens(
    ...
    (tester) async {
      ...
      await screenMatchesGolden(tester, 'restaurants_error_state');
    },
  );

  testGoldens(
    'RestaurantsWidget - Loaded state with one restaurant',
    (tester) async {
      const tRestaurant = Link(
        id: 'id',
      ...
      when(() => mockLinksCubit.state).thenAnswer(
        (invocation) => const StaticLinksState.loaded([tRestaurant]),
      );
      when(() => mockLinksCubit.getLinks()).thenAnswer((_) async {
        return;
      });
      ...
      await screenMatchesGolden(
        tester,
        'restaurants_loaded_state_one_restaurant',
      );
    },
  );

  testGoldens(
    'RestaurantsWidget - Loaded state with many restaurants',
    (tester) async {
      const tRestaurant = Link(
        id: 'id',
        name: 'Tournament of Kings',
      ...
      when(() => mockLinksCubit.state).thenAnswer(
        (invocation) =>
            StaticLinksState.loaded(List.generate(30, (index) => tRestaurant)),
      );
      when(() => mockLinksCubit.getLinks()).thenAnswer((_) async {
        return;
      });
      ...
      await screenMatchesGolden(
        tester,
        'restaurants_loaded_state_with_many_restaurants',
      );
    },
  );
}
